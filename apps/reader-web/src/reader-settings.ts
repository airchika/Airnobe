...
export const SHORTCUT_ACTIONS = [...READING_ACTIONS, "toggleSidebar"] as const;
...
export interface ReaderAppearance {
  ...
  typography: { fontSize: number; fontWeight: 400 | 600 | 800; lineHeight: number; paragraphSpacing: number; columnWidth: number; japaneseOpacity: number; rubyScale: number };
...
export const DEFAULT_SHORTCUTS: Record<ShortcutAction, ShortcutBinding> = {
  ...
  topBackward: { code: "KeyE" }, topForward: { code: "KeyF" }, bottomBackward: { code: "KeyW" }, bottomForward: { code: "KeyS" },
  pageUp: { code: "KeyA" }, pageDown: { code: "KeyD" }, toggleSidebar: { code: "KeyQ" },
};
export const DEFAULT_READER_SETTINGS: ReaderSettings = { version: 9, navigation: { textSteps: 2 }, shortcuts: cloneShortcuts(DEFAULT_SHORTCUTS), pageTransitions: false, appearance: structuredClone(DEFAULT_READER_APPEARANCE) };
...
export function parseReaderSettings(value: unknown): ReaderSettings | undefined {
  ...
  if (version === 9) {
    ...
    return { version: 9, navigation: { textSteps }, shortcuts, pageTransitions: typeof record.pageTransitions === "boolean" ? record.pageTransitions : false, appearance };
  }
  if (version === 8 || version === 7) {
    const appearance = parseReaderAppearance(record.appearance);
    if (!appearance) return undefined;
    return { version: 9, navigation: { textSteps }, shortcuts: cloneShortcuts(DEFAULT_SHORTCUTS), pageTransitions: Boolean(record.pageTransitions), appearance };
  }
...
export function cloneReaderSettings(settings: ReaderSettings): ReaderSettings { return { version: 9, navigation: { ...settings.navigation }, shortcuts: cloneShortcuts(settings.shortcuts), pageTransitions: settings.pageTransitions, appearance: structuredClone(settings.appearance) }; }
